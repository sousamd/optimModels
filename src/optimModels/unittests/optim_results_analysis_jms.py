...
def sol_str_to_list(str_sol, mode = ""):
    if mode == "KO":
        ...
        str_sol = str_sol.rstrip()
        str_sol = str_sol.replace("(0, ", "").replace(")", "").replace("'", "").replace(":", ",")
        str_sol = str_sol[1:-1].split(sep = ', ')
        for x in range(len(str_sol)):
            if x % 2 != 0:
                level = float(str_sol[x])
                if level > 1:
                    tag = " (O)"
                elif level == 0:
                    tag = " (KO)"
...
if __name__ == '__main__':
    ...
    csv_name2 = "wyield1500_UO_AERO_Gecko_SUCC_max15.csv"
    csv_name3 = "wyield1500_UO_AERO_Gecko_SUCC_max15_minmax_analysed.csv"
    data = pd.read_csv(csv_name3, sep = ";", skiprows = 2, skipfooter = 103, engine = "python")
    # data = pd.read_csv(csv_name3, sep = ";", skiprows = 2, skipfooter = 100, engine = "python")
    ...
    # data = data.sort_values('Fitness', ascending = False).sort_index()
    data = data[data['Fitness'] > 0.1].Reactions
    ...
    for solu in data:
        for prot in sol_str_to_list(solu, mode = "UO"):
    ...
    for protein in sol_counting:
        prot_code = protein[0].split(" ")[0]
        handle = urllib.request.urlopen("http://www.uniprot.org/uniprot/{}.xml".format(prot_code))
